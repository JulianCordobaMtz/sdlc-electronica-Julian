...
def create_reading(sensor_id: str, reading: SensorReadingIn, db: Session = Depends(get_db)):
...
def update_reading(reading_id: int, reading_update: SensorReadingUpdate, db: Session = Depends(get_db)):
